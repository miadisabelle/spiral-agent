...
export class NarrativeAnalysis {
     ...
     */
    async analyzePlotStructure(story) {
     ...
     */
    async analyzePacingAndRhythm(story) {
     ...
     */
    async analyzeCharacterDevelopment(story) {
    ...
    generatePlotObservations(story, analysis) {
    ...
    generatePacingObservations(story, analysis) {
    ...
    identifyCharacters(story) {
        ...
        return uniqueNames.filter(name => !['Chapter', 'The', 'She', 'He', 'It', 'Then', 'When', 'But'].includes(name));
    }
    analyzeCharacterArcs(story, characters) {
        // Simplified: check if character appears in beginning and end
        const firstQuarter = story.substring(0, story.length / 4);
        const lastQuarter = story.substring((story.length * 3) / 4);
        return {
            hasGrowth: characters.some(char => firstQuarter.includes(char) && lastQuarter.includes(char)),
            inconsistencies: []
        };
    }
    analyzeAuthenticity(story, characters) {
    ...
    detectMisbeliefs(story, characters) {
    ...
    generateCharacterObservations(story, analysis) {
...
//# sourceMappingURL=NarrativeAnalysis.js.map